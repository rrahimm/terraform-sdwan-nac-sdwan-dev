resource "sdwan_feature_device_template" "feature_device_template" {
  ...
  general_templates = flatten([
    ...
    try(each.value.cli_template, null) == null ? [] : [{
      ...
      try(each.value.sig_credentials_templates, null) == null ? [] : [for sct in try(each.value.sig_credentials_templates, []) : {
      id      = sdwan_cisco_sig_credentials_feature_template.sig_credentials_feature_template[sct.name].id
      version = sdwan_cisco_sig_credentials_feature_template.sig_credentials_feature_templatee[sct.name].version
